...
*   **Resolución DNS**: 
    ...
    *   **Público**: Se utiliza DuckDNS (`victorm2daw.duckdns.org`) para la validación de certificados SSL reales.

## 2. Creación de Usuarios de Despliegue
Para cumplir con la gestión de usuarios y permisos, se incluye un script de automatización `crear_usuario.sh`.
...
services:
  mi-app:
    image: nginx:alpine
    environment:
      - VIRTUAL_HOST=apps.victor.2daw,victorm2daw.duckdns.org
      - VIRTUAL_PORT=80
      - LETSENCRYPT_HOST=victorm2daw.duckdns.org
      - LETSENCRYPT_EMAIL=victor@correo.com
...
El servidor implementa **HTTPS Real** mediante Let's Encrypt.

*   **Configuración**: El contenedor `acme-companion` monitoriza las etiquetas `LETSENCRYPT_HOST` y utiliza el `DuckDNS_Token` configurado en el stack principal para emitir los certificados automáticamente.

## 5. Monitorización y Métricas
El sistema de monitorización es accesible vía web sin necesidad de puertos adicionales:
*   **Grafana**: `http://grafana.victor.2daw` (Usuario/Pass configurados).
*   **Prometheus**: `http://prometheus.victor.2daw`.
...
*   **Gestión Visual**: Acceder a `http://portainer.victor.2daw` para administrar contenedores, redes y volúmenes mediante interfaz gráfica.
